...
class RegistryDeploy extends SfCommand {
    ...
    async gatherUserInput(allComponents, allClasses) {
    ...
    // =================================================================
    async collectDependencies(depName, depType, params, seen = new Set()) {
        ...
        const subDeps = await Promise.all(dependencies.map((dep) => this.collectDependencies(dep.name, dep.type, params, seen)));
    ...
    // =================================================================
    async safeListDirNamesAsync(base) {
        try {
            const entries = await fs.readdir(base, { withFileTypes: true });
            return entries.filter(e => e.isDirectory()).map(e => e.name);
        }
        catch (error) {
            this.error(`❌ Erreur lors de la lecture du dossier "${base}" : ${error.message}`);
        }
    }
    ...
    async checkForbiddenFiles(dirPath) {
...
async function fileExistsAndIsFile(filePath) {
    try {
